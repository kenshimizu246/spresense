...
bool RGB88ToRBG565(uint8_t *src_buf, uint16_t *dst_buf, uint32_t src_len);
...
void loop()
{
    ...
    //    tft.drawRect(96, 0, 128 - 96, 10, ST77XX_BLACK)
    tft.fillScreen(ST77XX_BLACK);

    RGB88ToRBG565(ei_camera_capture_out, (uint16_t *)ei_camera_capture_out, EI_CLASSIFIER_INPUT_WIDTH * EI_CLASSIFIER_INPUT_HEIGHT * 3);
    tft.drawRGBBitmap(0, 0, (uint16_t *)ei_camera_capture_out, EI_CLASSIFIER_INPUT_HEIGHT, EI_CLASSIFIER_INPUT_WIDTH);
...
bool RGB88ToRBG565(uint8_t *src_buf, uint16_t *dst_buf, uint32_t src_len)
{
    ...
    for(uint32_t i = 0; i < pix_count; i ++) {
        ...
        c =  (rgb >> 8 & 0xf800) | (rgb >> 5 & 0x7e0) | (rgb >> 3 & 0x1f);
